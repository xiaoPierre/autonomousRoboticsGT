...
if __name__ == '__main__':
    opencv_dir = '/home/zhang/software/opencv/data/haarcascades/';
    ...
    def detect_and_draw(imgmsg, pub):
        ...
        for (x,y,w,h) in faces:
            # publish region of interest message
            s.append(RegionOfInterest(x_offset=x, y_offset=y, height=h, width=w))
            pub.publish(s)
        ...
        cv2.imshow('img',img)
        cv2.waitKey(10)
    pub = rospy.Publisher("regionOfInterest", ROSs)
    def detect_and_draw_and_publish(imgmsg):
        detect_and_draw(imgmsg, pub)
    rospy.Subscriber("/vrep/visionSensor", sensor_msgs.msg.Image, detect_and_draw_and_publish)
